...
import {
  ...
  ICustomer,
  UpdateCustomerBody,
} from "../types/customer.interfaces";
...
): Promise<ICustomerDoc | null> => Customer.findById(id);
...
export const getCustomerByEmail = async (
  email: string
): Promise<ICustomerDoc | null> => Customer.findOne({ email });
...
export const updateCustomerById = async (
  ...
  if (
    updateBody.email &&
    (await Customer.isEmailTaken(updateBody.email, customerId))
  ) {
    throw new ApiError(httpStatus.BAD_REQUEST, "Email already taken");
  }
